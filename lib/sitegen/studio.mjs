...
export const chosenStudio = () => STUDIOS[process.env.SITEGEN_STUDIO ?? "kimi"] ?? STUDIOS.kimi;
...
*/
async function viaOpenRouter({ studio, system, user, schema, schemaName, maxTokens, signal }) {
  ...
  try {
    ...
    throw new Error("The studio returned something that was not valid JSON.");
  }
...
async function viaAnthropic({ studio, system, user, zodSchema, maxTokens, signal }) {
  ...
  const response = await client.messages.parse({
    ...
    output_config: { format: zodOutputFormat(zodSchema, "site_spec") },
    signal,
  });
...
export async function artDirect({
  ...
  maxTokens = 16000, signal, studio = chosenStudio(),
}) {
  ...
  const raw = studio.via === "openrouter"
    ? await viaOpenRouter({ studio, system, user, schema: jsonSchema, schemaName, maxTokens, signal })
    : await viaAnthropic({ studio, system, user, zodSchema, maxTokens, signal });
