...
function chapterMarkup(c, i, photo = null, say = null, alt = null) {
  const w = say ? (a, b) => say(a, b) : (a) => esc(a);
  ...
  */
  ...
  const head = c.motion === "char-reveal"
    ? `<h2 class="ch-head split">${alt ? chars(c.headline) : chars(c.headline)}</h2>`
    : `<h2 class="ch-head">${w(c.headline, alt?.headline)}</h2>`;

  const body = c.body ? `<p class="ch-body">${w(c.body, alt?.body)}</p>` : "";
  ...
  <section class="scene scene--horizontal" id="${id}" data-motion="horizontal" data-chapter="${esc(c.name)}">
    ...
    <div class="scene-inner">
      <div class="h-head"><p class="ch-kicker">${w(c.kicker, alt?.kicker)}</p>${head}</div>
      <div class="h-track">
  ...
  <section class="scene" id="${id}" data-motion="counter" data-chapter="${esc(c.name)}">
    ...
    <div class="scene-inner center">
      <p class="ch-kicker">${w(c.kicker, alt?.kicker)}</p>
      <p class="counter" data-to="${Number(c.counterTo) || 0}">0</p>
      <p class="counter-label">${w(c.counterLabel, alt?.counterLabel)}</p>
      ${head}${body}
    </div>
  </section>`;
  }
...
export function renderSite(spec) {
  ...
  /*
    ...
    Emitted together rather than as two pages: a scroll-film is one canvas
    of pinned scenes, and building it twice would double the ScrollTriggers
    and the images for text that is a class toggle away. The inactive
    language is hidden with CSS, so the primary still reads correctly with
    no JavaScript at all.

    `t` takes the primary string and its translation and returns whichever
    markup the situation needs — a bare string when there is only one
    language, so a monolingual site carries no extra spans.
  */
  const lang = spec.language ?? { primary: "en", primaryLabel: "English", secondary: null, secondaryLabel: null };
  const bilingual = Boolean(lang.secondary && spec.alt);
  const say = (primary, alt) => (bilingual && alt
    ? `<span lang="${esc(lang.primary)}" data-l="a">${esc(primary)}</span>` +
      `<span lang="${esc(lang.secondary)}" data-l="b">${esc(alt)}</span>`
    : esc(primary));
  const A = spec.alt;
  /* CJK glyphs fill their em box in a way Latin display faces do not, so
     the tight leading that makes a Latin hero look carved makes a Chinese
     one look collided. Loosened only where it applies. */
  const cjk = /^(zh|ja|ko)/i.test(lang.primary);


  /*
    Photographs, and where each one is allowed to appear.

    Round-robin across the chapters is right when a client gave us a folder
    of their own work: two pictures over six beats reads as a motif, and the
    page stays photographic throughout.

    It is wrong for a single image. One picture round-robinned is the SAME
    picture behind every scene, top to bottom — which is what a generated
    hero produced, and it looks like a bug because it is one.

    So a lone image is a hero and nothing else, and the scenes keep the
    gradient world they were designed with. Scene photography needs enough
    material to feel deliberate; below three it does not.
  */
  const photos = Array.isArray(spec.images) ? spec.images.filter(Boolean) : [];
  const heroPhoto = photos[0] ?? null;
  const sceneable = photos.length >= 3 ? photos : [];
  /*
    Chapters start at photos[1], not photos[0].

    photos[0] is the hero. Reading the pool from zero meant chapter one
    always repeated the hero image — the first thing a visitor did after
    looking at the hero was scroll down and see it again. With one generated
    image per beat the offset makes every section unique; with a handful of
    scraped photographs it still cycles, but no longer opens on a repeat.
  */
  const photoFor = (i) => (sceneable.length ? sceneable[(i + 1) % sceneable.length] : null);
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* One language visible at a time. The primary is the default, so a
     reader with no JavaScript sees a complete site in the language their
     business actually speaks. */
  [data-l="b"]{display:none}
  html[data-alt] [data-l="a"]{display:none}
  html[data-alt] [data-l="b"]{display:inline}
  /* Bottom-right, because every other corner is taken: the wordmark holds
     top-left, the chapter readout top-right, the scroll cue bottom-centre
     and the progress rail the right edge at mid-height. The first attempt
     put this top-right and it landed on the readout. */
  .langtog{position:fixed;bottom:1.4rem;right:clamp(1.2rem,4vw,3rem);z-index:30;
    background:none;border:1px solid ${esc(p.ink)}33;color:var(--ink);border-radius:2rem;
    padding:.35rem .85rem;font:inherit;font-size:.72rem;letter-spacing:.06em;cursor:pointer;
    min-height:36px;transition:border-color .3s,background .3s}
  .langtog:hover{border-color:var(--accent);background:${esc(p.accent)}14}
  @media(max-width:700px){.langtog{bottom:1rem;right:1rem}}
  ...
  /* The strongest picture, behind the first thing anyone sees. Photographs
     were only on the chapter scenes, which are all below the fold — so a
     visitor landed on a gradient and concluded the site had no imagery.
     Loaded eagerly, unlike the scenes: this one is the first impression and
     must not fade in late. */
  .hero-photo img{transform:scale(1.04)}
  .hero-photo::after{background:
    radial-gradient(70% 60% at 50% 45%, ${esc(p.bg)}b3, ${esc(p.bg)}66 60%, ${esc(p.bg)}d9)}
  ...
  .hero h1{font-family:var(--display);font-weight:400;font-size:clamp(2.8rem,11vw,8rem);line-height:${cjk ? '1.16' : '.94'};letter-spacing:${cjk ? '0' : '-.03em'}}
  ...
  .ch-head{font-family:var(--display);font-weight:400;font-size:clamp(1.9rem,5.4vw,3.6rem);line-height:${cjk ? '1.3' : '1.08'};letter-spacing:${cjk ? '0' : '-.02em'}}
...
<body>
${bilingual ? `<button class="langtog" id="langtog" type="button"
  aria-label="Switch language">${esc(lang.secondaryLabel ?? "EN")}</button>` : ""}
...
<main id="top">
  <header class="hero" id="hero">
    ${heroPhoto ? `<div class="scene-photo hero-photo"><img src="${esc(heroPhoto)}" alt="" fetchpriority="high" decoding="async" /></div>` : ""}
    <div class="hero-glow" data-drift="1"></div>
    <div class="hero-glow2" data-drift="2"></div>
    <div class="hero-inner">
      <h1 class="split">${chars(spec.brandName)}</h1>
      <p class="tag">${say(spec.tagline, A?.tagline)}</p>
    </div>
    <p class="cue"><span></span>Scroll</p>
  </header>

${spec.chapters.map((c, i) => chapterMarkup(c, i, photoFor(i), say, A?.chapters?.[i] ?? null)).join("\n")}

  <div class="after" id="after">
    <div class="wrap">
${spec.sections.map((s, si) => `      <section class="sect">
        <h3>${say(s.title, A?.sections?.[si]?.title)}</h3>
        <p class="lede">${say(s.body, A?.sections?.[si]?.body)}</p>
        ${s.items.length ? `<div class="grid">
${s.items.map((it, ii) => `          <article class="card"><h4>${say(it.heading, A?.sections?.[si]?.items?.[ii]?.heading)}</h4><p>${say(it.text, A?.sections?.[si]?.items?.[ii]?.text)}</p></article>`).join("\n")}
        </div>` : ""}
      </section>`).join("\n")}

      <section class="cta">
        <h3>${say(spec.cta.heading, A?.cta?.heading)}</h3>
        <p>${say(spec.cta.body, A?.cta?.body)}</p>
        <a class="btn" href="#top">${say(spec.cta.label, A?.cta?.label)}</a>
      </section>
    </div>
  ...
  ${contactMarkup(spec.contact)}
    ...
    <footer>
      <div class="wrap">
        ...
        <span>${say(spec.footerNote, A?.footerNote)}</span>
      </div>
      <!--
        The credit.
...
(function () {
  ...
  gsap.utils.toArray('.scene').forEach(function (scene) {
    ...
    /*
      How long the words stay up.

      Each scene pins for 110% of the viewport and its copy fades out before
      the pin releases; the next scene's copy only begins once that scene
      reaches the top. Between the two there was a screen-height of scroll
      with a photograph and no words on it — four times over a five-chapter
      film, about a fifth of the page. Read as missing text rather than as a
      transition, which is a fair reading.

      Measured cause, not a guess: at the point a new scene fills 98% of the
      screen its copy was still at opacity 0, because the entrance only
      begins when the scene pins. A quarter of a screen of scrolling passed
      before any words arrived.

      So the entrance is short (ENTER), the hold is most of the pin (HOLD)
      and the exit is quick (EXIT) — copy readable across ~70% of each
      scene's scroll rather than ~53%. The beat between scenes survives —
      it should, it is a film — but it is a beat now rather than a gap.
    */
    var ENTER = 0.28, HOLD = '+=1.5', EXIT = 0.32;
    ...
    if (motion === 'pin-zoom') {
      tl.fromTo(inner, { scale: .86, opacity: 0, y: 40 }, { scale: 1, opacity: 1, y: 0, duration: ENTER, ease: 'power2.out' })
        .to(inner, { scale: 1.12, opacity: 0, y: -40, duration: EXIT, ease: 'power2.in' }, HOLD);
    } else if (motion === 'clip-reveal') {
      tl.fromTo(inner, { clipPath: 'inset(0 0 100% 0)', opacity: .2 },
                       { clipPath: 'inset(0 0 0% 0)', opacity: 1, duration: ENTER, ease: 'power2.out' })
        .to(inner, { clipPath: 'inset(100% 0 0 0)', opacity: 0, duration: EXIT, ease: 'power2.in' }, HOLD);
    } else if (motion === 'char-reveal') {
      var cs = scene.querySelectorAll('.ch-head .ch');
      tl.from(cs, { yPercent: 115, opacity: 0, duration: ENTER, stagger: .008, ease: 'power3.out' })
        .from(scene.querySelectorAll('.ch-kicker, .ch-body'), { y: 18, opacity: 0, duration: ENTER, stagger: .06 }, '<.15')
        .to(inner, { opacity: 0, y: -30, duration: EXIT, ease: 'power2.in' }, HOLD);
    } else if (motion === 'counter') {
      var el = scene.querySelector('.counter');
      var target = Number(el.dataset.to) || 0;
      var box = { v: 0 };
      tl.fromTo(inner, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: ENTER })
        .to(box, { v: target, ease: 'none',
                   onUpdate: function () { el.textContent = Math.round(box.v).toLocaleString(); } }, '<')
        .to(inner, { opacity: 0, y: -30, duration: EXIT, ease: 'power2.in' }, HOLD);
    } else { /* layer-parallax */
      tl.fromTo(inner, { opacity: 0, y: 60 }, { opacity: 1, y: 0, duration: ENTER, ease: 'power2.out' })
        .to(inner, { opacity: 0, y: -60, duration: EXIT, ease: 'power2.in' }, HOLD);
    }
  ...
  /* The toggle. Sets an attribute on <html>; the CSS does the rest, so
   nothing re-renders and no scroll position is lost. */
(function(){
  var b = document.getElementById('langtog');
  if (!b) return;
  var A = ${JSON.stringify(lang.primaryLabel)}, B = ${JSON.stringify(lang.secondaryLabel ?? "EN")};
  var langA = ${JSON.stringify(lang.primary)}, langB = ${JSON.stringify(lang.secondary ?? "en")};
  b.addEventListener('click', function(){
    var on = document.documentElement.hasAttribute('data-alt');
    if (on) { document.documentElement.removeAttribute('data-alt'); document.documentElement.lang = langA; b.textContent = B; }
    else { document.documentElement.setAttribute('data-alt',''); document.documentElement.lang = langB; b.textContent = A; }
