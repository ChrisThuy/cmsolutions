...
*/
/*
  Two image models, and which one a keyframe goes to.

  Soul 2.0 is the only one that takes a trained Soul id, so it stays the
  choice when a client's own photographs have been trained into one. Without
  that it is the wrong tool here: on a 16:9 canvas it answered close-up
  barbering prompts by tiling the frame into two portrait panels — the same
  scene at two moments, side by side. Three keyframes in a row came back that
  way, and it survived every attempt to prompt around it: naming the fault
  ("no split screen, no panels") only fed it, and rewriting the prompt as a
  single frozen instant with no camera move changed nothing.

  Nano Banana Pro returned one coherent photograph from the identical prompt.
  It costs two credits against Soul's 0.12, which on eight keyframes is about
  sixteen credits — real money, and cheaper than a film that cannot be shown.
*/
export function imageArgs({ prompt, quality = "2k", references = [], soulId = null }) {
  ...
  // Up to fourteen, but one is what the pipeline has: the client's own room
  // is a stronger anchor for the world than any amount of description.
  for (const ref of references.slice(0, 14)) args.push(`--image=${ref}`);
  args.push("--json");
  return args;
}
