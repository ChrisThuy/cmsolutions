...
export function buildBusiness(input) {
  ...
  const address = prune({
    ...
    /*
      No fallback. This used to default to "GB", which meant a business that
      typed a Singapore address and never touched the country field published
      structured data placing it in the United Kingdom. A country code the
      user did not enter is invented data, and this tool refuses to invent
...
export function validate(input, document) {
  ...
  // An address without a country is ambiguous to a search engine — but
  // guessing one is worse, so this asks instead of assuming.
  const hasAddressParts = Boolean(
    clean(input.street) || clean(input.city) || clean(input.postcode),
  );
  if (hasAddressParts && !clean(input.country)) {
    warnings.push(
      "No country code on the address. Add the two-letter code (SG, GB, US, AE…) — without it search engines have to guess where you are, and this tool will not guess for you.",
    );
  }
  if (clean(input.country) && !/^[A-Za-z]{2}$/.test(clean(input.country))) {
    errors.push(
      `"${clean(input.country)}" is not a two-letter country code. Schema.org expects ISO 3166-1 alpha-2, such as SG or GB.`,
    );
  }
