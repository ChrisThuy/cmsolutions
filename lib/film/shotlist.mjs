...
export const ShotSchema = z.object({
  ...
  visual: z.string(),
  /*
    The same shot as one frozen instant, for the keyframe.

    `visual` is written for a video model, so it carries time: "camera tracks
    along the comb line", "section after section", "as he reaches the door".
    Handed to a still-image model, that is a request for several moments in
    one picture — and it obliges, by splitting the canvas and drawing two
    panels side by side. Three separate keyframes came back as diptychs
    before this field existed, and telling the model not to split the frame
    did nothing, because the prompt was still describing a sequence.

    One instant. No camera move, no "then", no "as", no repetition.
  */
  still: z.string(),
  /*
...
4. MOVEMENT, AND THE FROZEN VERSION OF IT. Every shot needs one deliberate
   camera move in "visual" — drifting in, rising, tracking along, settling.
   Never "dynamic" or "energetic": say what the camera does.

   Then write "still": the same shot as a single photograph, at one instant.
   No camera movement, no "then", no "as he…", nothing happening twice. Not
   "the comb lifts a section and the scissors close on it, section after
   section" but "the scissors closed on a lifted section of hair, cut strands
   on the cape". The still is generated by an image model, and a description
   containing two moments comes back as two pictures side by side in one
   frame — which is what happened, three times, before this was split out.
  ...
  */
  ...
  return {
    ...
    chapters: shotlist.shots.map((shot) => {
      ...
      const context = `${constant} ${tidy(shot.wardrobe)} ` +
                      `No text, no lettering, no signage, no logos, no watermark.`;
      return {
        name: shot.name,
        /* For the video model: carries the camera move. */
        visual: `${shot.visual} ${context}`,
        /* For the image model: one instant, no movement. */
        still: `${shot.still ?? shot.visual} ${context}`,
      };
    }),
  };
}
