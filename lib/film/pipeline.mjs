...
import { generateClip as higgsfieldClip } from "../media/higgsfield.mjs";
...
export async function buildFilm({
  ...
  /*
    Higgsfield pins both ends, so the chain is a different shape.

    Each clip is given the frame it must start on AND the frame it must
    finish on, which means the junction is a picture both neighbours were
    handed rather than a drift discovered afterwards. That needs every
    boundary frame to exist before any clip is generated, so the keyframes
    are made first — one per chapter boundary, chapters + 1 of them.

    The gate still runs. Pinning is an instruction, not a guarantee, and a
    seam that was promised and not delivered is exactly the thing worth
    measuring.
  */
  if (engine === "higgsfield") {
    const boundaries = [opening.url];
    for (let i = 1; i <= chapters.length; i++) {
      ...
      const frame = await generateImage({
        prompt: `${chapter.visual}. Cinematic still, ${spec.conceptName ?? ""}. ` +
                `Same world, lens and palette as the rest of the film. No text, no titles, no watermark.`,
        tier: "draft",
        size: "landscape_16_9",
      });
    ...
    for (const [i, chapter] of chapters.entries()) {
      ...
      const clip = await higgsfieldClip({
