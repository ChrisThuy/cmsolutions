/*
  Methane regulation, as obligations an operator can be assessed against.

  ── why this file is written the way it is ─────────────────────────────────

  The audience for this knows the subject better than the tool does. A single
  invented requirement, a wrong date, or a penalty figure nobody can source
  destroys the credibility of everything else on the screen — and it will be
  spotted in the room, not later.

  So three rules govern every entry below:

    1. Every obligation carries a `source`. If it cannot be attributed, it does
       not go in.
    2. Dates are stated as published. Where a rule is in flux — and US federal
       methane policy is in flux — that is said rather than smoothed over.
    3. Nothing is quantified that cannot be cited. No "typical operators save
       £X", no modelled fine exposure. The regulation's own numbers, or
       nothing.

  This produces a readiness indicator, not a compliance determination and not
  legal advice. It is deliberately incapable of generating a regulatory
  submission: telling someone they are ready when they are not would be worse
  than telling them nothing, and quantifying methane is a measurement problem
  that belongs to instrumentation and qualified people, not to a web form.

  LAST REVIEWED: 2026-08-05. Methane rules are moving quickly in several
  jurisdictions. Anything here older than a quarter should be re-checked
  against the sources before it is shown to anyone.
*/

export const LAST_REVIEWED = "2026-08-05";

export const SOURCES = {
  /*
    Every citation here is primary — the Official Journal, the Federal
    Register, a UN body, or the Library of Congress. An earlier version cited
    trade newsletters and a law-school blog for two dates, which is fine for
    orientation and indefensible in a room that has read the regulation. The
    test suite now fails if a source is not on a government or IGO domain.
  */
  euReg: {
    label: "Regulation (EU) 2024/1787 — Methane Regulation, consolidated text",
    url: "https://eur-lex.europa.eu/eli/reg/2024/1787/oj/eng",
  },
  euDatabase: {
    label: "Regulation (EU) 2024/1787, Article 30 — methane transparency database",
    url: "https://eur-lex.europa.eu/eli/reg/2024/1787/oj/eng",
  },
  euImport: {
    label: "Regulation (EU) 2024/1787, Articles 27–29 — obligations on importers",
    url: "https://eur-lex.europa.eu/eli/reg/2024/1787/oj/eng",
  },
  ogmp: {
    // The UNEP page this used to cite is a 404. The partnership runs its own
    // site and is the authority on what its levels mean; this page states the
    // Level 5 reconciliation requirement the finding above describes.
    label: "OGMP 2.0 — reporting levels and Level 5 requirements",
    url: "https://www.ogmpartnership.org/frequently-asked-questions",
  },
  epa: {
    label: "EPA final rule, 91 FR 9712 — GHGRP reporting deadline for 2025 extended to 30 October 2026",
    url: "https://www.federalregister.gov/documents/2026/02/27/2026-03995/extending-the-reporting-deadline-under-the-greenhouse-gas-reporting-rule-for-2025",
  },
};

/** Where an operation sits relative to the EU, which is what drives most of this. */
export const EU_RELATIONSHIPS = [
  { value: "eu-producer", label: "We produce or operate assets inside the EU" },
  { value: "exporter", label: "We export oil, gas or LNG into the EU" },
  { value: "supplier", label: "We supply someone who exports into the EU" },
  { value: "importer", label: "We import oil, gas or LNG into the EU" },
  { value: "none", label: "No EU production, exports or imports" },
];

/*
  What the organisation actually does.

  This matters more than segment. An upstream operator's methane problem is a
  measurement programme; a trader's is whether the cargo they just bought can
  still be sold into Europe in 2027, and a financier's is which counterparties
  carry that risk. Asking a trading desk about their LDAR schedule is how a
  tool announces it was built for somebody else.
*/
export const ROLES = [
  { value: "operator", label: "We operate producing assets" },
  { value: "trader", label: "We trade or market crude, products or LNG" },
  { value: "refiner", label: "We refine or process" },
  { value: "shipping", label: "We own or charter vessels" },
  { value: "financier", label: "We finance, invest in or insure the above" },
];

export const SEGMENTS = [
  { value: "upstream", label: "Upstream — production" },
  { value: "midstream", label: "Midstream — gathering, processing, transport" },
  { value: "lng", label: "LNG — liquefaction, shipping or regasification" },
  { value: "integrated", label: "Integrated across several of these" },
  { value: "services", label: "Oilfield services or equipment" },
];

export const OGMP_LEVELS = [
  { value: "none", label: "Not an OGMP 2.0 member" },
  { value: "l1-2", label: "Level 1–2 — reporting at asset level or by source category" },
  { value: "l3", label: "Level 3 — source-level, generic emission factors" },
  { value: "l4", label: "Level 4 — source-level, own measurements" },
  { value: "l5", label: "Level 5 — source-level reconciled against site-level measurement" },
  { value: "unknown", label: "Not sure" },
];

/*
  The obligations themselves.

  `applies` decides whether an obligation is in scope for a given profile —
  telling an operator about rules that do not touch them is noise, and noise is
  how a report stops being read.

  `assess` returns "ready", "gap" or "unknown" from their answers. "unknown" is
  a first-class outcome: not knowing whether you comply is itself the finding,
  and it is the most common honest answer in this space.
*/
export const OBLIGATIONS = [
  {
    id: "eu-mrv-producer",
    title: "Measurement, reporting and verification at EU producer standard",
    regime: "EU Methane Regulation",
    date: "2027-01-01",
    dateNote: "Applies to new import contracts from 2027; EU operators are already inside scope.",
    source: "euReg",
    detail:
      "The regulation requires quantification of methane at both source level and site level, with independent third-party verification. Source-level estimates alone do not meet it.",
    applies: (p) => p.euRelationship !== "none",
    assess: (p) => {
      if (p.siteLevelMeasurement === "unknown" || p.verification === "unknown") return "unknown";
      if (p.siteLevelMeasurement === "yes" && p.verification === "yes") return "ready";
      return "gap";
    },
    ifGap:
      "Site-level measurement reconciled against a source-level inventory, and an independent verifier engaged. This is the obligation with the longest lead time — instrumentation, a measurement campaign and a verification cycle are not things that can be arranged in a quarter.",
  },
  {
    id: "ogmp-l5",
    title: "OGMP 2.0 Level 5, or a demonstrably equivalent standard",
    regime: "OGMP 2.0 / EU equivalence",
    date: "2027-01-01",
    dateNote: "The practical benchmark the EU has pointed to for import equivalence.",
    source: "ogmp",
    detail:
      "Level 5 means source-level emission estimates reconciled with site-level measurements. The Commission has indicated that reporting demonstrably equivalent to Level 5, with independent verification, is the route to import equivalence.",
    applies: (p) => p.euRelationship !== "none",
    assess: (p) => {
      if (p.ogmpLevel === "unknown") return "unknown";
      if (p.ogmpLevel === "l5") return "ready";
      return "gap";
    },
    ifGap:
      "A staged path from where you are to Level 5. Operators at Level 3 are usually one measurement programme away; operators not in OGMP at all are further out than the calendar suggests.",
  },
  {
    id: "eu-database",
    title: "Data ready for the EU Methane Transparency Database",
    regime: "EU Methane Regulation",
    date: "2026-09-01",
    dateNote:
      "Article 30(1) required the Commission to establish the database by 5 February 2026; it was not live on that date and launch has been reported for September 2026. The duty in Article 30 is the Commission's — what falls on a supplier is the exposure that begins when it goes live.",
    source: "euDatabase",
    detail:
      "Once live, the database makes the methane performance of suppliers into the EU visible and comparable. Importers and buyers will be able to see who reports what. This is a commercial exposure rather than a filing deadline, which is why it is worth being ready before the date rather than on it.",
    applies: (p) => p.euRelationship !== "none",
    assess: (p) => {
      if (p.inventory === "unknown") return "unknown";
      return p.inventory === "yes" ? "ready" : "gap";
    },
    ifGap:
      "A defensible source-level inventory you would be willing to have compared with your peers in public. The commercial risk here is not a fine — it is a buyer reading the comparison.",
  },
  {
    id: "ldar",
    title: "A leak detection and repair programme with documented surveys",
    regime: "EU Methane Regulation",
    date: "2027-01-01",
    dateNote: "Survey types and frequencies are set out in the regulation and its implementing acts; confirm the schedule that applies to your asset class.",
    source: "euReg",
    detail:
      "LDAR is the operational backbone of the regulation: recurring surveys, documented findings, and evidence that what was found was repaired within the required window.",
    applies: (p) => p.euRelationship !== "none" && p.segment !== "services",
    assess: (p) => {
      if (p.ldar === "unknown") return "unknown";
      if (p.ldar === "programme") return "ready";
      return "gap";
    },
    ifGap:
      "Surveys on a defined schedule, with findings and repairs recorded in a form an auditor can follow. Ad-hoc inspection does not produce the evidence trail the regulation asks for.",
  },
  {
    id: "intensity",
    title: "Knowing your methane intensity",
    regime: "EU Methane Regulation",
    date: "2028-01-01",
    dateNote: "Intensity reporting under the Commission's methodology; maximum intensity thresholds follow in 2030.",
    source: "euImport",
    detail:
      "From 2028 producers report methane intensity using an EC-established methodology, and from 2030 imported oil, gas and coal must sit below maximum intensity thresholds.",
    applies: (p) => ["eu-producer", "exporter", "importer"].includes(p.euRelationship),
    assess: (p) => {
      if (p.intensity === "unknown") return "unknown";
      return p.intensity === "yes" ? "ready" : "gap";
    },
    ifGap:
      "Intensity cannot be calculated without the inventory and the production data lined up against each other. Operators usually discover the gap is in reconciling the two, not in either one.",
  },
  {
    id: "supply-chain",
    title: "Methane data from your own suppliers",
    regime: "EU Methane Regulation",
    date: "2027-01-01",
    dateNote: "Importers must show equivalence for what they bring in, which means evidence from upstream.",
    source: "euImport",
    detail:
      "Equivalence is assessed on the imported stream. That obligation passes up the chain, and in pooled markets where several producers' gas is commingled before liquefaction it is materially harder to evidence.",
    applies: (p) => ["importer", "lng"].includes(p.euRelationship) || p.segment === "lng",
    assess: (p) => {
      if (p.supplierData === "unknown") return "unknown";
      return p.supplierData === "yes" ? "ready" : "gap";
    },
    ifGap:
      "Contractual data requirements with suppliers, and somewhere to put what comes back. This is usually a commercial and systems problem rather than a measurement one.",
  },
  {
    id: "us-ghgrp",
    title: "US Greenhouse Gas Reporting Program, Subpart W",
    regime: "US EPA",
    date: "2026-10-30",
    dateNote: "Reports for calendar year 2025 emissions, deadline extended to 30 October 2026.",
    source: "epa",
    detail:
      "The reporting deadline for calendar year 2025 is 30 October 2026, set by a final rule (91 FR 9712) and unchanged by its June 2026 correction. Two things are moving around it and neither removes it: the Waste Emissions Charge has been delayed to reporting year 2034 by P.L. 119-21, and EPA has proposed — not finalised — removing the gas distribution segment from Subpart W and suspending the remaining segments until 2034. Until that proposal is final, the 2026 report is still due.",
    applies: (p) => p.usOperations === "yes",
    assess: (p) => {
      if (p.usReporting === "unknown") return "unknown";
      return p.usReporting === "yes" ? "ready" : "gap";
    },
    ifGap:
      "This one has a fixed near date and is a reporting exercise rather than a measurement programme, which makes it the most tractable item on most operators' lists.",
  },
];

/*
  The commercial obligations.

  These are the ones that bite a desk rather than a field. They are not softer
  than the operational obligations — from 2030 a cargo that cannot evidence its
  intensity is a cargo that cannot be sold into the EU, which is a trading
  problem long before it is a compliance one.
*/
const COMMERCIAL = [
  {
    id: "cargo-evidence",
    title: "Evidence of methane performance for what you buy and sell",
    regime: "EU Methane Regulation",
    date: "2027-01-01",
    dateNote: "Equivalence is assessed on the imported stream, from 2027 for new contracts.",
    source: "euImport",
    detail:
      "Equivalence attaches to the molecule, not to the company holding it. A cargo whose upstream methane performance cannot be evidenced is a cargo whose route into the EU narrows, whoever is selling it.",
    applies: (p) => ["trader", "refiner", "shipping"].includes(p.role) && p.euRelationship !== "none",
    assess: (p) => {
      if (p.cargoEvidence === "unknown") return "unknown";
      return p.cargoEvidence === "yes" ? "ready" : "gap";
    },
    ifGap:
      "A way to receive, store and pass on methane data alongside the cargo — the same discipline already applied to quality and origin. Most desks find the data exists somewhere upstream and simply does not travel with the trade.",
  },
  {
    id: "contract-clauses",
    title: "Purchase contracts that oblige suppliers to provide MRV data",
    regime: "EU Methane Regulation",
    date: "2027-01-01",
    dateNote: "New import contracts from 2027 are the trigger, so contracts written now are already in scope.",
    source: "euReg",
    detail:
      "Data you have no contractual right to is data you will not get when you need it. Contracts being signed today will still be running when equivalence is assessed.",
    applies: (p) => ["trader", "refiner", "financier"].includes(p.role) && p.euRelationship !== "none",
    assess: (p) => {
      if (p.contractClauses === "unknown") return "unknown";
      return p.contractClauses === "yes" ? "ready" : "gap";
    },
    ifGap:
      "Standard clauses requiring measurement-based methane reporting from counterparties, and a place to put what arrives. This is the cheapest item on the list and the one with the longest tail if it is left.",
  },
  {
    id: "counterparty-view",
    title: "A view of which counterparties can actually evidence their methane",
    regime: "EU Methane Regulation / commercial",
    date: "2026-09-01",
    dateNote: "The Transparency Database makes supplier reporting visible from September 2026.",
    source: "euDatabase",
    detail:
      "Once supplier performance is public and comparable, it becomes a differentiator between cargoes and a screening question for lenders and insurers. Knowing where your counterparties sit before the market does is a commercial advantage with a short shelf life.",
    applies: (p) => ["trader", "refiner", "financier", "shipping"].includes(p.role),
    assess: (p) => {
      if (p.counterpartyView === "unknown") return "unknown";
      return p.counterpartyView === "yes" ? "ready" : "gap";
    },
    ifGap:
      "A counterparty register carrying methane reporting status alongside the credit and compliance fields you already track. The data is largely public; what is missing is somewhere to keep it and a reason to look.",
  },
  {
    id: "intensity-threshold",
    title: "Exposure to the 2030 maximum intensity thresholds",
    regime: "EU Methane Regulation",
    date: "2030-01-01",
    dateNote: "From 2030 imported oil, gas and coal must sit below maximum methane intensity thresholds.",
    source: "euImport",
    detail:
      "This is the one that changes what can be traded rather than what must be reported. Long-term supply signed today runs into it, and the intensity of a barrel is decided upstream by people who may not yet be measuring.",
    applies: (p) => ["trader", "refiner", "financier"].includes(p.role) && p.euRelationship !== "none",
    assess: (p) => {
      if (p.thresholdExposure === "unknown") return "unknown";
      return p.thresholdExposure === "yes" ? "ready" : "gap";
    },
    ifGap:
      "An assessment of which supply in your book would clear a threshold and which would not. Four years sounds comfortable until it is set against the tenor of the contracts already on the sheet.",
  },
];

OBLIGATIONS.push(...COMMERCIAL);

/*
  Operational obligations belong to operators.

  Bolted on here rather than written into each `applies` above so the rule is
  stated once and visibly: a trading desk is not asked about LDAR schedules or
  site-level measurement campaigns, because those are not theirs to run.
*/
const OPERATIONAL_ONLY = new Set(["eu-mrv-producer", "ogmp-l5", "ldar", "intensity", "eu-database"]);
for (const obligation of OBLIGATIONS) {
  if (!OPERATIONAL_ONLY.has(obligation.id)) continue;
  const original = obligation.applies;
  obligation.applies = (p) => (p.role ? p.role === "operator" : true) && original(p);
}

/** Obligations in scope for a profile, soonest deadline first. */
export function applicableObligations(profile) {
  return OBLIGATIONS.filter((o) => o.applies(profile)).sort((a, b) =>
    a.date.localeCompare(b.date),
  );
}
