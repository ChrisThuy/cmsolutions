...
/**
 * Positions findings on a timeline from today to the last deadline.
 *
 * The report is a wall of text, and a compliance calendar is inherently a
 * picture: four years, five dates, and a position on it. Someone glancing at a
 * screen takes the shape in before they read a word, which is the difference
 * between being read and being walked past.
 *
 * Returns positions as percentages so the renderer stays dumb and this stays
 * testable. Nothing here is decorative — every mark is a published date.
 */
export function timeline(result, today = new Date()) {
  const dated = result.findings.filter((f) => f.date);
  if (!dated.length) return null;

  const start = Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate());
  const times = dated.map((f) => new Date(`${f.date}T00:00:00Z`).getTime());
  const last = Math.max(...times);

  // A span of at least a year, so a single near deadline does not produce a
  // timeline where everything sits on top of everything else.
  const end = Math.max(last, start + 365 * 86_400_000);
  const span = end - start || 1;

  // Deadlines are shared — several obligations fall on 2027-01-01 — so they
  // are grouped, otherwise the labels collide and the picture stops helping.
  const byDate = new Map();
  for (const f of dated) {
    const entry = byDate.get(f.date) ?? { date: f.date, items: [], horizon: f.horizon };
    entry.items.push({ title: f.title, status: f.status });
    byDate.set(f.date, entry);
  }

  const marks = [...byDate.values()]
    .map((entry) => {
      const at = new Date(`${entry.date}T00:00:00Z`).getTime();
      const open = entry.items.filter((i) => i.status !== "ready").length;
      return {
        date: entry.date,
        horizon: entry.horizon,
        position: Math.max(0, Math.min(100, ((at - start) / span) * 100)),
        count: entry.items.length,
        open,
        // The worst status at that date drives the colour: one open item is
        // not softened by two beside it that are in hand.
        status: open === 0 ? "ready" : entry.items.some((i) => i.status === "gap") ? "gap" : "unknown",
      };
    })
